...
nb-web treats notebooks as the unit of import and export. The **NbWeb-archive** plugin handles archiving, importing, and distributing notebooks as `.nbz` files.
...
> The **Remove notebook** function lives in the notebook's **Danger Zone**, not here. Archive first if you have unpushed commits — the Danger Zone warns you.
...
If the notebook has unpushed commits the Danger Zone shows a warning with the count — archive first if those commits matter.
