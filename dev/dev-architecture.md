...
| `foldable:` | string or list | Makes matching headings collapsible. Values treated as RegExp (case-insensitive); tested against raw heading line including `#` prefix. Plain strings match as literal substrings; single-quote for regex special chars. Fold state persisted in localStorage. See [[docs:foldable.md]]. |
