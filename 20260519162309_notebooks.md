...
nb-web does not yet have a "New notebook" button — create notebooks from the CLI:

```bash
nb notebooks add <name>
```

The new notebook appears in the list immediately on the next Notebooks page load. Then wire a remote from the detail panel if you want to sync it.
